...
> **面向 Claude Code 与 Codex 的轻量级执行治理层，让 AI 编程工作有计划、可审计、真的经过验证。**
...
Claude-Praxis 是给 AI 编程代理使用的一层治理系统。

它不替代 Claude Code 或 Codex，而是给它们一套更有纪律的工作方式：

1. 先理解真实目标，而不是只照着字面 prompt 执行
2. 根据任务大小选择执行模式
3. 把计划、交接、验证写进持久文件
4. 大任务使用子代理时限制边界
5. 声称完成之前先验证
...
简单说：它把编程代理从「我试着改了，然后告诉你我做了什么」变成「我先框定目标，写计划，分阶段执行，验证结果，并留下证据链」。

Claude-Praxis 现在同时支持：

| 代理 | 全局入口 | 全局安装路径 | 项目工作区 |
|---|---|---|---|
...
> "重构支付重试逻辑，并确保 checkout 仍然可用。"

没有 Praxis 时，代理可能直接冲进代码，做出看起来合理的修改，跑一个顺手的测试，然后说「完成」。如果上下文很长，它可能忘记之前为什么选了某个方案；如果用了子代理，交接过程可能不可见；如果跳过了验证，你只能事后发现。

有 Praxis 时，代理会被推入一个可见流程：

| 阶段 | Praxis 让什么变得可见 |
|---|---|
| 编辑前 | 这是 lightweight、standard、deep 还是 recovery 任务？ |
| 规划时 | 真实目标是什么？哪些不做？哪里可能失败？ |
| 执行中 | 哪些文件、交接、决策、风险发生了变化？ |
| 验证时 | 哪个命令或检查真正证明结果有效？ |
| 完成时 | 证据文件在哪里？完成声明是否指向该证据？ |

实际效果是：更少「看起来完成」的失败，更少上下文丢失，更安全的子代理使用，以及能跨长会话保存的项目记忆。
...
> "修复 invoice 表格里的日期格式 bug。"

如果它真的只是一个小修改，Praxis 会把它判定为 `lightweight`。代理可以直接改文件并做基本检查，不会为一个单文件修复制造仪式感。

### 例 2：一次有风险的重构

Prompt：

> "重构 auth middleware，让移动端和网页端共享同一套 session 校验。"

Praxis 会把它判定为 `standard` 或 `deep`。代理会在项目工作区写计划，记录假设，跟踪风险，并验证真实入口，而不是只检查 TypeScript 能不能编译。

### 例 3：一个长周期多代理任务

Prompt：

> "把这个 app 从旧数据库 schema 迁移到新 schema，并保留现有用户数据。"

Praxis 会要求有边界的交接。一个子代理可以检查 schema 使用情况，另一个可以审查迁移风险，主代理再把结果整合进持久计划。即使上下文压缩，关键状态也在文件里，而不是只留在聊天记录中。
...
Praxis 会自动判定模式，在需要时写下 plan-as-files，大任务使用范围受限的子代理，对照真实目标做验证，并输出耦合证据的 closure token。
...
在任意项目中打开 Codex，执行同类真实任务。Codex-Praxis 使用 `AGENTS.md`，安装到 `~/.codex/`，并将持久工作区约定映射到 `<repo>/.codex/`。
...
- **一行更新** —— `~/.claude/install.sh --update` 或 `~/.codex/install-codex.sh --update`

---

## Latest — v1.5.0

v1.5.0 上线 **控制硬化** —— `praxis doctor` 支持纯 JSON 控制输出，规则元数据由 `rules.json` 提供，closure token 可通过 SHA-256 绑定证据文件，handoff 检查也变为 mode-aware。

```bash
praxis doctor check --json --brief                 # agent 可读取的工作区判定
praxis doctor verify-closure <plan-id> --json --brief
praxis doctor list-rules --json                    # 从 rules.json 列出规则
```

关闭令牌现在支持 `sha256=<evidence-file-hash>` 与 `verifier=PASS`。审计阅读者可复现该判定，并在最后一行未变化时仍检测证据内容是否被修改。

| 层 | v1.x 机制 | v1.5 状态 |
|---|---|---|
| 外化 | 计划、mode rubric、日志、证据文件 | 高 |
| 反思 | 强制读取、self-eval、证据指针 | 中 |
| 验证 | **`praxis doctor` CLI + `rules.json` + 稳定 rule ID** | doctor 规则覆盖处可验证；其余处可观测 |
...
Claude-Praxis 将随意的 Claude Code 或 Codex 会话转化为结构化、可审计、可恢复的工作。它在编码代理之上添加了一层轻薄的运作层：目标框定、模式选择、持久化规划、范围受限的子代理、验证证据，以及跨上下文压缩的连续性保障。

---

## 为何存在

Claude Code 和 Codex 本身已经很强大。问题不在于能力，而在于真实工作中的操作纪律：
...
Claude-Praxis 补充了缺失的控制层，而不取代编码代理的原生工具。
...
**机制：** 在采取任何首个行动之前，标准 / 深度 / 恢复模式任务必须在 `<project-workspace>/_meta/mode-decisions/<plan-id>.md` 写入一个模式决策文件，符合 `MODE_DECISION_SCHEMA.md` 规范。该 schema 包含一个量化标准：文件数量、工具调用估算、领域数量、生产路径暴露情况、多阶段标志以及其他可测量的标准。分类决策从这些数字推导，而非依靠印象性判断。
...
**制品位置：** `<project-workspace>/_meta/mode-decisions/<plan-id>.md`
...
v1.5+ 的关闭令牌应加入 `sha256=<evidence-file-hash>` 与 `verifier=PASS`。`last-line` 字段必须包含所引用证据文件的实际最后一个非空行；`sha256` 将令牌绑定到完整证据内容。

**为何选择此方案而非备选：** 纪律层面的规则（"声明完成前必须验证"）在完成压力下会失效。它们也无法核实：遵守规则的消息与真正经过验证的消息在外观上完全相同。格式层面的规则将声明与证据在语法层面耦合；加入 `sha256` 后，验证器还能发现关闭之后的证据内容修改。

**轻量级模式直接输出"done"。** 关闭令牌仅适用于需要计划即文件的正式工作。

**制品位置：** `<project-workspace>/_meta/validation/closure-<plan-id>.md`
...
**制品位置：** `<project-workspace>/logs/execution-log.md`、`<project-workspace>/validation/self-evaluation.md`
...
### v1.5 已部分交付：3D — 阶段边界审计

**当前状态：** `praxis doctor verify-closure` 已能重新读取证据，并验证 `last-line` 与可选 `sha256`。剩余的 3D 工作是自动阶段边界派发；验证器本身已经存在。
...
- **关闭令牌可伪造性已缩小，但未消失。** `praxis doctor` 可验证 `last-line` 与 `sha256`，但如果周围控制流不要求在关闭前运行 `--json --brief` 验证，代理仍可能跳过它。
...
这些不是缺陷，而是选择只读验证而非硬执法所付出的代价。Praxis 在 doctor 规则覆盖处可验证，在其余处可观测。针对残余风险的防御措施是：在 `metrics/` 中积累证据、同行审查、doctor JSON 控制流，以及对执行日志进行人工检查。
...
| 文件 | 作用 |
|---|---|
| `CLAUDE.md` | Claude Code 全局入口点和执行模式规则 |
| `AGENTS.md` | Codex 全局入口点和执行模式规则 |
...
| `CODEX_INTEGRATION.md` | 与 Codex 原生入口、工具和工作区路径的映射 |
...
| `install.sh` | Claude Code 幂等安装程序和完整性检查器 |
| `install-codex.sh` | Codex 幂等安装程序和完整性检查器 |
...
对于非简单工作，Praxis 会创建或使用项目本地的 `<project-workspace>`。

对于 Codex，同一结构使用 `.codex/`；具体路径映射参见 `CODEX_INTEGRATION.md`。
...
Codex-Praxis 会安装 `AGENTS.md` 和协议文件；不会覆盖 `~/.codex/config.toml`、`auth.json`、会话日志、插件或 SQLite 状态。
